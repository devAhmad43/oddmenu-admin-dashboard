  ...
  const handleChange = async (status, id) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${serverUrl}/api/order/updatestatus/${id}`,
        {
          status: changestatus,
        }
      );
  ...
  return (
    <>
      ...
      <div className="min-w-full overflow-x-auto">
        <h2 className="mr-3 font-bold text-2xl text-center mb-4 mt-1.5  text-purple-700">
          All Orders
        </h2>
          ...
          <table className="min-w-full bg-white divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th
                  scope="col"
                  className="px-3 py-3 text-center min-w-[150px] text-md font-bold uppercase tracking-wider"
                >
                  Table No
                </th>
                <th
                  scope="col"
                  className="px-3 py-3 text-center min-w-[200px] text-md font-bold uppercase tracking-wider"
                >
                  Total Price
                </th>
                <th
                ...
                <th
                  scope="col"
                  className="px-3 py-3 text-center min-w-[100px] text-md font-bold uppercase tracking-wider"
                >
                  Payment Status
                </th>
                <th
              ...
              {myorders && myorders.length > 0 ? (
                myorders.map((user, index) => (
                  <tr key={index} className="text-center">
                    <td
                      title="details"
                      className="px-6 py-4 whitespace-nowrap text-sm font-bold curser-pointer text-purple-900"
                    >
                      <Link to={`/Admin/order/${user._id}`}>
                        {user.tableNumber}
                      </Link>
                    </td>
                    ...
                    <td className="font-medium text-gray-900">
                      ...
                      >
                        <option
                          selected={user.status === true}
                          value="completed"
                        >
                          Completed
                        </option>
                        <option
                          selected={user.status === false}
                          value="pending"
                        >
                          Pending
                        </option>
